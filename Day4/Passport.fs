...
type Passport(fields: Field array) =
    ...
    member this.FieldNames = Set.ofArray (fields |> Array.map (fun a -> a.Name))
...
let isValid (passport: Passport) =
    requiredFields |> List.fold (fun a b -> a && passport.FieldNames.Contains(b)) true
...
let isValidField (field: Field) =
    ...
    | { Name = "hgt" } -> match field.Value with
                            | Regex @"^(\d+)cm$" [height] -> between height 150 193
                            | Regex @"^(\d+)in$" [height] -> between height 59 76
                            | _ -> false
    | { Name = "hcl" } -> match field.Value with
                            | Regex @"^#[0-9a-f]{6}$" _ -> true
                            | _ -> false
    | { Name = "ecl" } -> match field.Value with
                            | Regex @"^amb|blu|brn|gry|grn|hzl|oth$" _ -> true
                            | _ -> false
    | { Name = "pid" } ->  match field.Value with
                            | Regex @"^\d{9}$" _ -> true
                            | _ -> false
    | _ -> true

let isStrictValid (passport: Passport) =
    isValid passport && passport.Fields |> Array.fold (fun a b -> a && isValidField b) true